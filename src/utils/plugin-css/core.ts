...
export async function pluginCSS<TConfig extends object>(
	...
	if ('getEnvironmentInfo' in firstParam && config) {
	  ...
	  if (cssAlreadyImported) {
		config.debug && console.log(`[${plugin.pluginId}] CSS already imported, skipping`);
		return;
	  }
	  ...
	  if (env.isBundlerEnvironment) {
		...
		console.warn(`[${plugin.pluginId}] CSS autoloading is disabled in bundler environments. Please import the CSS manually, using import.`);
	  }
	  return;
	}
